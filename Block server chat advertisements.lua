...
local interface = ffi.cast('uintptr_t**', client.create_interface('client.dll', 'VClient018'))
...
-- Create a menu checkbox to toggle
local enabled = ui.new_checkbox('Misc', 'Miscellaneous', 'Block server chat advertisements')
...
local function hkDispatch(this, type, flags, size, data)
  ...
  return (blocked_messages[type] and ui.get(enabled)) and true or oDispatch(this, type, flags, size, data)
end
